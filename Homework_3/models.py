...
class RandomForestLearningAlgorithm(BaseLearningAlgorithm):
    ...
    def __init__(
        self, n_estimators: int, criterion: str, max_depth: int, min_samples_split: int
    ):
...
class DeepNeuralNetworkLearningAlgorithm(BaseLearningAlgorithm):
    ...
    def compute_average_gradient(self, batch_gradients):
        ...
        for key in batch_gradients[0].keys():
            average_gradient[key] = np.mean(
                [grad[key] for grad in batch_gradients], axis=0
            )
    ...
    def backward_pass(self, y_train: np.array, output: np.array) -> Dict[str, np.array]:
        ...
        # Calculate W2 update
        error = np.dot(params["W3"].T, error) * self.activation(
            params["Z2"], derivative=True
        )
        change_w["W2"] = np.outer(error, params["A1"])

        # Calculate W1 update
        error = np.dot(params["W2"].T, error) * self.activation(
    ...
    def predict(self, X_test: np.array) -> np.array:
        ...
        return np.array([str(np.argmax(self.forward_pass(x))) for x in X_test]).astype(
            "float32"
        )
    ...
    def fit(self, x_train, y_train, x_val, y_val):
        ...
        for iteration in range(self.epochs):
            ...
            print(
                "Epoch: {0}, Time Spent: {1:.2f}s, Validation Accuracy: {2:.2f}%".format(
...
class KerasDnnLearningAlgorithm(BaseLearningAlgorithm):
    ...
    def _get_model(self, input_dim, output_dim, hidden_layers, activation):
        ...
        # input layer
        model.add(
            tf.keras.layers.Dense(input_dim, input_dim=input_dim, activation=activation)
        )

        # hidden and droput layers
        for layer_width in hidden_layers:
            model.add(tf.keras.layers.Dense(layer_width, activation=activation))
